...
// https agent - SSL 검증 무시 (일부 서버에서 필요)
...
app.get('/api/dams/trend', async (req, res) => {
  ...
  try {
    ...
    res.json({ success:true, id, obscd, name:info.name, minL:info.minL, maxL:info.maxL, full:info.full, data:sampled });
  } catch(e) {
    res.status(502).json({ success:false, error:e.message });
  }
});
...
app.get('/api/health', (req, res) => {
  res.json({ status:'ok', version:'v14', time:new Date().toISOString(), damCount: Object.keys(DAM_OBS).length });
});

app.listen(PORT, () => {
  console.log(`DAM VOLUME MASTER v14 — http://localhost:${PORT}`);
});
